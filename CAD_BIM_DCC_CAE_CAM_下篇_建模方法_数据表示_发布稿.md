...
> 核心判断：同一个三维形状进入五类软件后，会被重写为五种不同的数据对象。
...
![NURBS 曲面与控制点](素材图片/014_CAD_Modeling_Paradigms__NURBS,_Parametric,_Implicit,_SubD/001.png)

*NURBS 用控制点和数学曲面描述连续外形；B-Rep 则进一步组织面、边、顶点及其拓扑关系。图片来自素材 14。*
...
![参数化特征树驱动复杂装配体](素材图片/014_CAD_Modeling_Paradigms__NURBS,_Parametric,_Implicit,_SubD/002.png)

*参数化 CAD 不只保存最终外形，还保存特征顺序、约束和装配依赖。图片来自素材 14。*
...
![CAD 细节清理、简化与网格化](素材图片/035_CADfix_CAE__CAD_Model_Preparation_for_Simulation/001.jpg)

*面向 CAE 的几何准备通常要经历去特征、修复、简化和网格化，而不是把制造模型原样送入求解器。图片来自素材 35。*
...
![Revit、Paramodel、ueBIM 与 MSCE 的实体造型路径对比](素材图片/043_建筑信息模型三维建模软件的应用比较/012.jpg)

*相关学术素材从造型、约束、参数和规则等角度比较了多种 BIM 建模工具，说明 BIM 建模并非只有一种实现路径。图片来自素材 43。*
...
STEP 属于 ISO 10303 系列标准。ISO 官方对 ISO 10303-1:2024 的定义，是规定该系列中产品信息表示与交换的架构原则；在具体应用中，STEP 用于异构工程系统之间交换产品模型数据。STEP AP242 则重点覆盖精确 B-Rep、装配结构、产品元数据和语义 PMI/GD&T。

它适合 CAD 到 CAD 的供应商交换和长期归档，但不能保证完整保留原软件的特征树、约束历史和所有设计意图。

### IFC：面向建筑信息交换

IFC 是 buildingSMART 面向建成环境行业的开放、全球性、供应商中立标准，并被发布为 ISO 16739。它不仅描述几何，也描述建筑空间结构、构件对象、属性集合、数量和关系。截至本文资料整理时，buildingSMART 列出的正式版本为 IFC 4.3.2.0，对应 ISO 16739-1:2024。

STEP AP242 与 IFC 不是谁取代谁的关系。前者主要面向机械产品模型和 PMI，后者主要面向建筑、基础设施、机电和设施管理信息。把机械 CAD 数据直接塞进 IFC，或把 BIM 模型当成普通网格导出，都会损失语义。

### OBJ、FBX、glTF 面向资产交接，USD 面向场景组合

OBJ 适合静态网格，FBX 常用于带材质、骨骼和动画的 DCC 交接，glTF/GLB 适合 Web、AR 和实时预览。USD 不宜只被归入“另一种网格格式”：OpenUSD 官方将其定义为用于协同构建三维场景的高性能、可扩展平台，可用几何、材质、灯光和物理等模式在 DCC 工具之间交换数据，并通过组合机制把资产组织成更大的场景。

这些格式可以帮助资产跨工具流动，但不等于 CAD 的 B-Rep、BIM 的族关系或 CAM 的制造意图都能被无损翻译。

![Universal Scene Description 场景资产示意](素材图片/053_VFX_Software_in_2026__The_Honest_Map_of_Every_DCC_Tool/003.jpg)

*USD 的关键不只是保存单个模型，而是引用、分层和组合来自不同工具与部门的场景资产。图片来自素材 53。*
...
71. buildingSMART，Industry Foundation Classes (IFC) 官方定义  
    https://www.buildingsmart.org/standards/bsi-standards/industry-foundation-classes/

72. ISO，ISO 10303-1:2024 — Industrial automation systems and integration — Product data representation and exchange  
    https://www.iso.org/standard/83105.html

73. Alliance for OpenUSD，OpenUSD 官方文档  
